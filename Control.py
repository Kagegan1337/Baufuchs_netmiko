...
class PageTwo():
    def __init__(self, parent):
        ...
        labelMain = tk.Label(self.page_frame, width=24, text="Options", font=("Arial Bold", 15)).grid()
        usernameLabel = tk.Label(self.page_frame, text="Username").grid()
        usernameEntry = tk.Entry(self.page_frame, width=15).grid()
        self.page_frame.grid(row=0, column=0, sticky="ns")

        """
        lbl = Label(parent, text="Options", font=("Arial Bold", 15)).pack()
        
        # Frames
        self.usernameFrame = Frame(parent)
        self.usernameFrame.pack()

        self.passwordFrame = Frame(parent)
        self.passwordFrame.pack()

        self.buttonFrame = Frame(parent)
        self.buttonFrame.pack()

        self.testFrame = Frame(parent)
        self.testFrame.pack()
        ...
        self.usernameEntry = Entry(self.usernameFrame, width=15)
        self.usernameEntry.pack(side=RIGHT)

        # Password
        self.passwordLabel = Label(self.passwordFrame, text="Passwort")
        self.passwordLabel.pack(side=LEFT)

        self.passwordEntry = Entry(self.passwordFrame, show="*", width=15)
        self.passwordEntry.pack(side=RIGHT)

        def callback():
            a = self.usernameEntry.get()
            # testLabel.config(text=a)

        # Buttons
        self.btn = Button(self.buttonFrame, text="Login", command=callback).pack(side=LEFT)
        # btn = Button(buttonFrame, text="Cancel").pack(side=RIGHT)

        # Testlabel
        self.testLabel = Label(self.testFrame, text="test")
